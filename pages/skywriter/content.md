...
| **Data**     | Structured data in JSON or YAML, accessible via <%raw%>`<%= data.property %>`<%endraw%> |
...
| `data.yaml` / `data.json` | No       | Structured data accessible via <%raw%>`<%= data.property %>`<%endraw%> |
...
Templates wrap your content with shared layouts — headers, footers, navigation, etc. A template is itself a page that uses the <%raw%>`<%= slot %>`<%endraw%> variables to include child content.
...
<%raw%>- `<%~ slot.html %>` — The rendered HTML of the child page
...
- `<%~ slot.script.tag %>` — The child page's script tag<%endraw%>
...
| Syntax              | Purpose                     |
| ------------------- | --------------------------- | --------------------- |
| <%raw%>             | `<%= expression %>`         | Output (HTML-escaped) |
| `<%~ expression %>` | Output (raw/unescaped HTML) |
| `<% code %>`        | Execute JavaScript logic    | <%endraw%>            |
...
<%raw%>- `<%= title %>` — The page title

- `<%= path %>` — The current page path
- `<%= meta %>` — Metadata object (`createdAt`, `updatedAt`, `toc`, `headings`)
- `<%= data %>` — Parsed JSON/YAML from the Data tab<%endraw%>

#### Dynamic Content

<%raw%>- `<%= server %>` — Data returned from your `server.js` default export
...
- `<%~ markdown %>` — Rendered Markdown content (available in templates)<%endraw%>

#### Style Helpers

<%raw%>- `<%= style.content %>` — Raw CSS

- `<%~ style.inlineTag %>` — `<style>...</style>` tag
- `<%= style.href %>` — URL to the CSS file
- `<%~ style.tag %>` — `<link rel="stylesheet" href="...">` tag<%endraw%>

#### Script Helpers

<%raw%>- `<%= script.content %>` — Raw JavaScript

- `<%~ script.inlineTag %>` — `<script>...</script>` tag
- `<%= script.href %>` — URL to the JS file
- `<%~ script.tag %>` — `<script src="..."></script>` tag<%endraw%>

#### Slot Variables

All the above are also available for slots via `slot.*`:

<%raw%>- `<%= slot.title %>`, `<%= slot.path %>`, `<%= slot.data %>`

- `<%~ slot.html %>`, `<%~ slot.markdown %>`
- `<%~ slot.style.tag %>`, `<%~ slot.script.tag %>`<%endraw%>
