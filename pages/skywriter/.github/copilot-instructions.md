...
- When default export is a function (sync or async): Function is invoked with same variables as `skywriter render` command, and `server` equals the resolved return value
...
skywriter render | jq -r '[paths(scalars)] | map([.[] | if type == "number" then "[]" else . end] | join(".") | gsub("\\.\\[\\]\\."; ".[].") | gsub("\\.\\[\\]$"; "[]")) | unique | .[]'
```
